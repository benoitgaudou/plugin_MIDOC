...
species bus_stop skills: [TransportStopSkill] schedules:bus_stop where (each.is_start_stop) {
    ...
    action update_next_trip() {
		next_trip_index <- next_trip_index + 1 ;    	
...
experiment Toulouse type: gui parent: GTFSExperiment {
	...
	parameter "Bus start stop" var: selected_bus_stop <- 1033;
}
...
experiment Nantes type: gui parent: GTFSExperiment {
	...
	parameter "Bus start stop" var: selected_bus_stop <- 79; //"ABDO2"
}
...
experiment Hanoi type: gui parent: GTFSExperiment {
	...
	parameter "Bus start stop" var: selected_bus_stop <- 0; 

}

experiment "3 villes" type: gui parent: GTFSExperiment {
	action _init_() {
		create simulation(gtfs_f_path: "../../includes/tisseo_gtfs_v2", boundary_shp_path: "../../includes/shapeFileToulouse.shp",selected_bus_stop: 1033);
		create simulation(gtfs_f_path: "../../includes/nantes_gtfs", boundary_shp_path: "../../includes/shapeFileNantes.shp", selected_bus_stop: 79);
		create simulation(gtfs_f_path: "../../includes/hanoi_gtfs_pm", boundary_shp_path: "../../includes/shapeFileHanoishp.shp", selected_bus_stop: 0);
	}
}
